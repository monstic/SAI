        ...
        if (Memory.rooms[roomName]) {
            ...
            for (var path in paths) {
                if (paths[path].lastused > (Game.time - 16) && paths[path].lastused < (Game.time - 10)) {
                    new RoomVisual(room.name).text('.', (paths[path].x), (paths[path].y + 0.05), { size: 2, opacity: 0.8, color: '#ffff00' });
                }
                if (paths[path].lastused > (Game.time - 11) && paths[path].lastused < (Game.time - 5)) {
                    new RoomVisual(room.name).text('.', (paths[path].x), (paths[path].y + 0.05), { size: 1.5, opacity: 0.8, color: '#ffff00' });
                }
